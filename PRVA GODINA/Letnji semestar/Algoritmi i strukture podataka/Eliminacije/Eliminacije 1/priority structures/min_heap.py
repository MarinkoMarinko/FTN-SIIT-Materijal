...
class MinHeap():
    ...
    def _upheap(self, index):
        ...
        if self._elements[parent]._key > self._elements[index]._key:
    ...
    def _downheap(self, index):
        if self._has_left(index):
            min_index = self._left(index)
            if self._has_right(index):
                right_index = self._right(index)
                if self._elements[right_index]._key < self._elements[min_index]._key:
                    min_index = right_index
                
            if self._elements[min_index]._key < self._elements[index]._key:
                self._elements[min_index], self._elements[index] =  self._elements[index], self._elements[min_index]
